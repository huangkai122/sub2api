...
const allNullQuotas: DefaultPlatformQuotasMap = {
  ...
  ark: { daily: null, weekly: null, monthly: null },
}
...
describe("normalizePlatformQuotasMap", () => {
  it("填充缺失的平台为全 null 三档", () => {
    ...
    expect(result.mimo).toEqual({ daily: null, weekly: null, monthly: null });
    expect(result.ark).toEqual({ daily: null, weekly: null, monthly: null });
  });
